...
function maximalMarginalRelevance(candidates, topK, lambda) {
...
function buildFusionMaps(denseList, bm25, queryText, method, weights) {
    ...
    const useLexical = method === "hybrid" && bm25 !== null && bm25.docCount > 0;
    if (useLexical) {
    ...
    const label = useLexical ? "hybrid" : "semantic";
...
export function retrieveRanked(index, bm25, config, queryText, queryEmbedding) {
    return retrieveRankedWithOptions(index, bm25, config, queryText, queryEmbedding, {});
}
/** Full fused ranking after path filter (no top_k / MMR truncation). Used for multi-query merge. */
export function retrieveFusedRankedUnbounded(index, bm25, config, queryText, queryEmbedding, options) {
...
export function retrieveRankedWithOptions(index, bm25, config, queryText, queryEmbedding, options) {
    const topK = config.retrieval.top_k;
    const ranked = retrieveFusedRankedUnbounded(index, bm25, config, queryText, queryEmbedding, options);
...
export function mergeMultiQueryScores(perQuery) {
    const best = new Map();
...
export function rankHitsFromMap(hits, topK, candidatePool, mmrEnabled, mmrLambda) {
