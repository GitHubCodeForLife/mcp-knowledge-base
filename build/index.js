...
async function main() {
    ...
    if (!process.env.OPENAI_API_KEY) {
        logInfo("OPENAI_API_KEY is not set; using deterministic hash embeddings (not true semantic). Set OPENAI_API_KEY for OpenAI embeddings.");
    }
    ...
    const index = await buildVectorIndex(corpus.map((c) => ({
        id: c.id,
        relativePath: c.relativePath,
        chunkIndex: c.chunkIndex,
        text: c.text,
    })), embedder);
    const bm25 = config.retrieval.method === "hybrid"
        ? Bm25Index.fromChunks(corpus.map((c) => ({ id: c.id, text: c.text })))
        : null;
