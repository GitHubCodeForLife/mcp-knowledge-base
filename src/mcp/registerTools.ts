...
function stripLeadingMountPrefix(input: string, mountName: string): string {
  const trimmed = input.trim();
  if (!trimmed) return trimmed;
  const normalized = trimmed.replace(/\\/g, "/");
  const prefix = `${mountName}/`;
  if (normalized === mountName) return "";
...
function getMountRoot(config: AppConfig, mountName: string): string | null {
  return config.resolvedSources.find((s) => s.name === mountName)?.absolutePath ?? null;
}

function ensureWikiEntryPoints(config: AppConfig): void {
  ...
  if (!fs.existsSync(indexPath)) {
    fs.writeFileSync(
      indexPath,
      ["# Index", "", "## Sources", "", "## Concepts", "", "## Entities", "", "## Analyses", ""].join("\n"),
      "utf8",
    );
  }
  if (!fs.existsSync(logPath)) {
    fs.writeFileSync(
      logPath,
      ["# Log", "", "Append-only operations log.", ""].join("\n"),
      "utf8",
    );
  }
}

function appendWikiLog(config: AppConfig, title: string, bullets: string[]): void {
  const wikiMount = config.knowledge_base.layers?.wiki_source ?? "wiki";
  const wikiRoot = getMountRoot(config, wikiMount);
  if (!wikiRoot) return;
  const logPath = path.join(wikiRoot, "log.md");
...
function updateWikiIndexAppend(
  config: AppConfig,
  sectionHeader: "## Sources" | "## Analyses",
  linkPath: string,
  description: string,
): void {
  const wikiMount = config.knowledge_base.layers?.wiki_source ?? "wiki";
  ...
  if (body.includes(line) || body.includes(`[\`${linkPath}\`]`)) return;
  ...
  server.registerTool(
    "update_document",
    {
      description: toolDescription(
        config,
        "update_document",
        "Update document content by file name (wiki-only)",
      ),
      inputSchema: {
        filename: z
          .string()
          .describe(
            "Path relative to a configured source mount (e.g. wiki/index.md). Only wiki/* is writable.",
          ),
  ...
  server.registerTool(
    "wiki_ingest_source",
    {
      description: toolDescription(
        config,
        "wiki_ingest_source",
        "Create/update a wiki source page, update wiki index and log",
      ),
      inputSchema: {
        source_filename: z
          .string()
    ...
    async ({ source_filename, summary_title, summary_markdown }) => {
      try {
        ...
        const rawResolved = resolveDocumentPath(
          config,
          stripLeadingMountPrefix(source_filename, rawMount),
        );
  ...
  server.registerTool(
    "wiki_file_answer",
    {
      description: toolDescription(
        config,
        "wiki_file_answer",
        "File a provided answer as a wiki analysis page and update index/log",
      ),
      inputSchema: {
          ...
          content: [
            {
              type: "text",
              text: JSON.stringify(
                { wiki_page: wikiPath, created_or_updated: [wikiPath, "wiki/index.md", "wiki/log.md"] },
                null,
                2,
              ),
            },
  ...
  server.registerTool(
    "wiki_query",
    {
      description: toolDescription(
        config,
        "wiki_query",
        "Search wiki-first and return top matching chunks (with optional raw fallback)",
      ),
      inputSchema: {
  ...
  server.registerTool(
    "wiki_lint",
    {
      description: toolDescription(
        config,
        "wiki_lint",
        "Check wiki health: missing entry points, orphans (not in index), contradiction markers",
      ),
      inputSchema: {
    ...
    async ({ include_details }) => {
      try {
        ...
        const indexPath = path.join(wikiRoot, "index.md");
        const logPath = path.join(wikiRoot, "log.md");
        const issues: Array<{ type: string; path?: string; message: string; suggestion?: string }> = [];
        ...
        const walk = (dir: string) => {
          for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
            ...
            else if (ent.isFile() && ent.name.toLowerCase().endsWith(".md")) mdFiles.push(abs);
          }
        };
        walk(wikiRoot);
